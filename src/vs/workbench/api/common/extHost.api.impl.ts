...
export function createApiFactoryAndRegisterActors(accessor: ServicesAccessor): IExtensionApiFactory {
	...
	return function (extension: IExtensionDescription, extensionInfo: IExtensionRegistries, configProvider: ExtHostConfigProvider): typeof vscode {
		...
		// namespace: window
		// Built-in chat was removed; the chat-panel/status members of the window
		// namespace are intentionally not provided. Omit them so the literal is
		// still contextually typed while satisfying the kept d.ts.
		const window: Omit<typeof vscode.window, 'activeChatPanelSessionResource' | 'onDidChangeActiveChatPanelSessionResource' | 'createChatStatusItem'> = {
