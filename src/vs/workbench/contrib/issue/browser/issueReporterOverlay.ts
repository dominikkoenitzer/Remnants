...
export class IssueReporterOverlay {
	...
	private createStep0Attachments(): void {
		...
		subtitle.textContent = localize('screenshotsSubtitle', "You can add up to {0} screenshots or videos. Navigate Remnants and choose when to capture.", MAX_ATTACHMENTS);
	...
	private createFloatingCaptureBar(): void {
		...
		// Mount inside .monaco-workbench so Remnants's color theme CSS vars
		...
		// Delay/options dropdown using Remnants's context menu
	...
	private updateReviewDetails(): void {
			...
			this.createDiagSection(diagContainer, {
				...
				renderContent: (container) => {
					...
					if (modelData.versionInfo) {
						this.addDiagRow(sysTable, 'Remnants', modelData.versionInfo.vscodeVersion);
		...
		if (modelData.extensionData) {
			...
			// Extension (e.g. built-in extensions are filed against Remnants),
	...
	private generateIssueDetailsMd(): string {
		...
		const rows: [string, string | undefined][] = [
			...
			['Remnants Version', modelData.versionInfo?.vscodeVersion ?? product.version],
	...
	private generateSystemInfoMd(): string {
		...
		if (modelData.versionInfo) {
			rows.push(
				['Remnants Version', modelData.versionInfo.vscodeVersion],
