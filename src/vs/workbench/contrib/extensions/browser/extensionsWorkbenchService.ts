...
**Notice:** This extension is bundled with Remnants. It can be disabled but not uninstalled.
## Features
${this.description}
`);
		}
	...
	async getChangelog(token: CancellationToken): Promise<string> {
		...
		if (this.type === ExtensionType.System) {
			return Promise.resolve(`Please check the [Remnants Release Notes](command:${ShowCurrentReleaseNotesActionId}) for changes to the built-in extensions.`);
		}
...
export class ExtensionsWorkbenchService extends Disposable implements IExtensionsWorkbenchService, IURLHandler {
	...
	async uninstall(e: IExtension): Promise<void> {
		...
		if (!defaultChatAgent || !areSameExtensions(extension.identifier, { id: defaultChatAgent.extensionId })) {
