		...
		const server = createServer((req, res) => {
			if (req.url === '/json/list' || req.url === '/json') {
				...
				res.end(JSON.stringify([{
					description: 'Remnants Renderer',
					devtoolsFrontendUrl: '',
					id: ident,
					title: 'Remnants Renderer',
					type: 'page',
					url: 'vscode://renderer',
		...
		const { server, wsUrl, port } = await this.openCdpServer(ident, listener => {
			...
			store.add(listener.onData(rawData => {
				...
				if (debugRenderer) {
					...
					const targetInfo = { targetId: ident, type: 'page', title: 'Remnants Renderer', url: 'vscode://renderer' };
